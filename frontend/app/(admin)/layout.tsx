...
export default function AdminLayout({
  ...
  const handleLogout = () => {
    // 1. Tước thẻ bài: Xóa Cookie (set ngày hết hạn về quá khứ)
    document.cookie = "token=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
    
    // 2. Dọn dẹp kho lưu trữ cục bộ (nếu có)
    localStorage.removeItem('token');
    
    // 3. Báo cáo thành công
    toast.success("Logged out successfully! See you again.");
    
    // 4. Trục xuất về trang Đăng nhập
    router.push('/login');
  };
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <aside className="w-64 bg-gray-900 text-white flex flex-col transition-all duration-300">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
          <Link href="/dashboard" className="flex items-center gap-3 px-4 py-3 text-gray-400 hover:bg-gray-800 hover:text-white rounded-lg transition-colors">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 z-10">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* NÚT LOGOUT ĐÃ ĐƯỢC GẮN SỰ KIỆN */}
            <button 
